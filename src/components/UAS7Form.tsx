...
export default function UAS7Form({
  ...
  return (
    <div id="uas7-form-capture" className="w-full bg-white dark:bg-gray-900 rounded-2xl p-4 sm:p-6 shadow-sm border border-rose-100 dark:border-slate-700/50">

      {/* Title & Progress Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h2 className="text-xl sm:text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight break-words">
      ...
      <div className="flex flex-wrap items-center gap-2 pb-5 mb-5 border-b border-gray-100 dark:border-gray-800 text-[11px] sm:text-xs text-gray-500 dark:text-gray-400">
        ...
        {SCORE_LABELS.map(s => (
          <span key={s.val} className={`px-2.5 py-1 rounded-md font-medium ${s.bg} whitespace-nowrap`}>
            {s.label} ({s.val})
          </span>
        ))}
      </div>
      ...
      <div className="overflow-x-auto -mx-4 sm:mx-0 px-4 sm:px-0">
        <div className="inline-block w-full align-middle">
          <table className="w-full border-separate border-spacing-y-2">
            <thead>
              <tr className="text-[9px] sm:text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">
                <th scope="col" className="text-left pb-2 pl-2 sm:pl-3">Data</th>
                <th scope="col" className="text-center pb-2 px-1 sm:px-4">
                  <span className="text-wine-700 dark:text-wine-400 block sm:inline">URTICÁRIAS</span>
                  <span className="text-[8px] sm:text-[9px] text-gray-400 block font-normal normal-case whitespace-normal leading-tight mt-0.5 mx-auto max-w-[100px] sm:max-w-none">
                ...
                <th scope="col" className="text-right pb-2 pr-2 sm:pr-3">PTs</th>
              </tr>
            </thead>
            ...
            <tbody>
              {weekDays.map((day, idx) => {
                ...
                return (
                  <tr
                    key={day.short}
                    className="bg-gray-50/50 dark:bg-gray-800/30 hover:bg-gray-100/50 dark:hover:bg-gray-800/60 rounded-xl transition-colors duration-150"
                  >
                    {/* Date label */}
                    <td className="py-2 sm:py-3 pl-2 sm:pl-4 rounded-l-xl min-w-[50px] sm:min-w-0">
                      <div className="font-bold text-gray-900 dark:text-white text-xs sm:text-base leading-tight">
                        {day.short}
                      </div>
                      <div className="text-[9px] sm:text-xs text-gray-400 dark:text-gray-500 font-medium">
                        {day.date}
                      </div>
                    </td>
                          ...
                          return (
                            <button
                              ...
                              className={`
                                ...
                                ${isSelected
                                  ? 'bg-wine-800 dark:bg-wine-700 text-white font-bold ring-2 ring-wine-300 dark:ring-wine-900 scale-105 shadow-sm'
                                  : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700'
                                }
                                rounded-full w-7 h-7 sm:w-auto sm:h-auto sm:px-4 sm:py-2 sm:rounded-lg text-[11px] sm:text-xs sm:font-medium
                              `}
                            >
                    ...
                    {/* Day Score Total */}
                    <td className="py-2 sm:py-3 pr-2 sm:pr-4 text-right rounded-r-xl font-bold text-gray-800 dark:text-gray-200 text-xs sm:text-base">
                      {daySum}
                    </td>
                  </tr>
                )
      ...
      <div className="mt-5 p-4 rounded-xl bg-wine-50 dark:bg-wine-950/30 border border-wine-100 dark:border-wine-900/40 flex items-center justify-between gap-4 transition-all duration-300">
        <div className="flex items-center gap-2">
          <svg className="w-5 h-5 text-wine-700 dark:text-wine-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 002 2h2a2 2 0 002-2z" />
          </svg>
          <span className="text-sm sm:text-base font-bold text-wine-800 dark:text-wine-300 leading-tight">
